...
const STATS = [
  { icon: '📦', num: '2,480+', labelKey: 'products' },
  { icon: '🏢', num: '340+',   labelKey: 'properties' },
  { icon: '🚗', num: '185+',   labelKey: 'vehicles' },
  { icon: '👥', num: '1,200+', labelKey: 'suppliers' },
] as const;

const FEATURED = [
  { icon: '📱', name: 'Samsung Galaxy A54 – 256GB',   price: '185,000 RWF',   loc: 'Kigali, Gasabo' },
  { icon: '🛺', name: 'Electric motorcycle – 2023',    price: '1,200,000 RWF', loc: 'Musanze' },
  { icon: '🌾', name: 'Maize grain – 50 kg bag',       price: '18,500 RWF',    loc: 'Rwamagana' },
];
    ...
    <div>
      ...
      >
        ...
        <aside className="hidden md:flex flex-col gap-3 shrink-0" style={{ width: 240 }}>
          ...
          {/* My Account */}
          <SideCard title="📋  My Account" titleBg={navy}>
            <SideLink href={user ? '/dashboard' : '/login'}>
              <span>👤 {user ? user.name.split(' ')[0] : `${T.signIn} / ${T.register}`}</span>
              <Chevron />
            </SideLink>
            <SideLink href={user ? '/my-listings' : '/login'}>
              <span>📋  My Listings</span><Badge>{user ? '→' : '0'}</Badge>
            </SideLink>
            <SideLink href={user ? '/coins' : '/login'}>
              <span>🪙  Coin Wallet</span><Badge>{user ? user.coins : '0'}</Badge>
            </SideLink>
            <SideLink href={user ? '/referral' : '/login'}>
              <span>🎁  Refer &amp; Earn</span><Chevron />
            </SideLink>
          </SideCard>

          {/* Sell */}
          <SideCard title="🏪  Sell on E-Nyagasambu" titleBg={navy}>
          ...
          {/* Live Auctions */}
          <SideCard title="🔨  Live Auctions" titleBg={darkOrg}>
        ...
        <div className="flex-1 flex flex-col gap-4 min-w-0">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {STATS.map(({ icon, num, labelKey }) => {
              const label = labelKey === 'suppliers'
              ...
              return (
                <div key={labelKey} className="bg-white border border-gray-200 rounded-lg p-3 text-center">
                  <div className="text-2xl mb-1" style={{ color: org }}>{icon}</div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {FEATURED.map(({ icon, name, price, loc }) => (
                <Link key={name} href="/listings"
                  ...
                  <div className="h-28 flex items-center justify-center bg-gray-50 text-4xl">{icon}</div>
                  <div className="p-2.5">
          ...
          <section className="bg-white border border-gray-200 rounded-lg p-4">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <RegBtn href="/register" primary>Ambassador Registration →</RegBtn>
              <RegBtn href="/certificate?type=broker" primary>Broker Portal →</RegBtn>
...
function SideCard({ title, titleBg, children }: { title: string; titleBg: string; children: React.ReactNode }) {
  return (
    <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
