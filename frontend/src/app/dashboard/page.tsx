        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, minWidth:160 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:7.5, color:'#666' }}>www.enyagasambu.rw</div>
            </div>
          </div>
        ...
        <div style={{ display:'flex', gap:14, alignItems:'flex-start' }}>
          ...
          <div style={{ flex:1, textAlign:'center' }}>
            ...
            <div style={{ display:'flex', justifyContent:'center', borderTop:'1px dashed #ccc', borderBottom:'1px dashed #ccc', padding:'6px 0', margin:'0 0 10px' }}>
              {[
                { icon:'📍', label:'AUTHORIZED TERRITORY', val:'Kigali City' },
                { icon:'📅', label:'ISSUE DATE', val:issued },
                { icon:'📅', label:'VALID UNTIL', val:validUntil },
...
export default function DashboardPage() {
  ...
  const certQr = user
    ? `https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=${encodeURIComponent(`https://enyagasambu.rw/verify/${certNo}`)}&bgcolor=ffffff&color=1B2A5E&margin=4`
    : '';
  ...
  return (
    <div className="min-h-screen" style={{ background: '#f0f2f9' }}>
      ...
      <div className="text-white px-6 py-8"
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <h1 className="text-2xl font-extrabold leading-tight">My Dashboard</h1>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            {isPriv && (
              <Link href="/staff"
                ...
                {user.role === 'admin' ? '🔐 Admin Panel' : '🛠️ Staff Panel'}
              </Link>
            )}
            <Link href="/listings/create"
              className="text-sm font-bold px-4 py-2 rounded-lg transition bg-white hover:opacity-90"
              style={{ color: NAVY }}>
              + Post Listing
            </Link>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        {/* ── STAFF/ADMIN: Platform stats row ── */}
        {isPriv && stats && (
          <section>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: 'Total Users',     value: stats.totalUsers,      icon: '👥', bg: NAVY },
                { label: 'Active Listings', value: stats.activeListings,  icon: '📋', bg: ORG },
                { label: 'Connects Made',   value: stats.totalUnlocks,    icon: '📞', bg: '#7c3aed' },
                { label: 'Coins Collected', value: (stats.coinsEarned||0)+(stats.coinsFromListings||0)+(stats.coinsFromBoosts||0), icon: '🪙', bg: '#059669' },
              ].map(card => (
                <div key={card.label} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                  ...
                  <p className="text-2xl font-extrabold" style={{ color: card.bg }}>
        ...
        <div className="flex gap-5" style={{ alignItems: 'flex-start' }}>
          ...
          {/* Left: Profile card */}
          <div className="flex flex-col gap-4 shrink-0" style={{ width: 230 }}>

            {/* Profile */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="h-14" style={{ background: `linear-gradient(90deg, ${NAVY}, ${ORG})` }}/>
              <div className="px-4 pb-4 -mt-7">
                <div
                  className="w-14 h-14 rounded-full flex items-center justify-center text-white font-extrabold text-lg border-4 border-white shadow-md mx-auto mb-3"
                  style={{ background: NAVY }}>
                  {initials}
                </div>
                <p className="font-bold text-sm text-center text-gray-900 truncate">{user.name}</p>
                <p className="text-xs text-center text-gray-400 truncate mb-2">{user.email}</p>
                <div className="flex justify-center mb-3">
                  <span className="text-xs font-bold px-2.5 py-0.5 rounded-full" style={{ background: badge.bg, color: badge.color }}>
                    {badge.label}
                  </span>
                </div>
                {/* Coin balance */}
                <div className="rounded-xl p-3 text-center" style={{ background: '#fff7ed' }}>
                  <p className="text-xs text-gray-500 mb-0.5">Coin Balance</p>
                  <p className="text-xl font-extrabold" style={{ color: ORG }}>🪙 {user.coins.toLocaleString()}</p>
                </div>
              </div>
            </div>
            ...
            {/* Quick links */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              <p className="text-xs font-bold uppercase tracking-widest px-4 py-2.5 border-b border-gray-100" style={{ color: NAVY }}>
                My Account
              </p>
              {[
                { href: '/dashboard',        icon: '🏠', label: 'Dashboard',       active: true },
                { href: '/my-listings',      icon: '📋', label: 'My Listings',     badge: listings.length },
                { href: '/coins',            icon: '🪙', label: 'Coin Wallet',      badge: user.coins },
                { href: '/subscriptions',    icon: '⭐', label: 'Seller Plans' },
                { href: '/referral',         icon: '🎁', label: 'Refer & Earn',     badge: referral?.totalReferrals ?? 0 },
                { href: '#ambassador-cert',              icon: '🏅', label: 'Ambassador Cert' },
                { href: '/certificate?type=broker',     icon: '🪪', label: 'Broker ID Card' },
              ].map(({ href, icon, label, badge: b, active }) => (
                <Link key={href} href={href}
                  className="flex items-center justify-between px-4 py-2.5 text-xs text-gray-700 border-b border-gray-50 last:border-b-0 hover:bg-gray-50 transition"
                  style={active ? { background: '#f0f2f9', color: NAVY, fontWeight: 700 } : {}}>
                  <span className="flex items-center gap-2">
                    <span>{icon}</span> {label}
                  </span>
                  {b !== undefined && (
                    <span className="text-[10px] px-2 py-0.5 rounded-full font-bold" style={{ background: '#e8ecf8', color: NAVY }}>
                      {b}
                    </span>
                  )}
                </Link>
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-4">
            ...
            <section>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { href: '/listings/create', icon: '➕', label: 'Post a Listing',   sub: 'Costs 400 coins' },
                  { href: '/coins',           icon: '🪙', label: 'Buy Coins',        sub: 'Top up wallet' },
                  { href: '/listings',        icon: '🔍', label: 'Browse Listings',  sub: 'Find products' },
                  { href: '/referral',        icon: '📨', label: 'Invite Friends',   sub: 'Earn 50 coins each' },
            ...
            <section>
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-[10px] text-gray-400">{myActive} active</span>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                  {listings.slice(0, 5).map((l, i) => (
                    <Link key={l.id} href={`/listings/${l.id}`}
                      className="flex items-center gap-3 px-4 py-3 border-b border-gray-50 last:border-b-0 hover:bg-gray-50 transition">
                      {/* thumb */}
                      <div className="w-12 h-12 rounded-lg overflow-hidden shrink-0 bg-gray-100 flex items-center justify-center">
                        {l.primary_image
                          ? <img src={`http://localhost:5000${l.primary_image}`} alt="" className="w-full h-full object-cover"/>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">
                          {l.category_name} · {l.price ? `${Number(l.price).toLocaleString()} RWF` : l.price_type}
                        </p>
                      </div>
                  ...
                  {listings.length > 5 && (
                    <Link href="/my-listings"
                      className="flex items-center justify-center py-2.5 text-xs font-semibold hover:bg-gray-50 transition"
                      style={{ color: ORG }}>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              {referral && (
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                  ...
                  <div className="flex justify-between mb-4">
                    {[
                      { label: 'Referrals',    value: referral.totalReferrals },
                      { label: 'Coins Earned', value: referral.bonusPaid },
                    ].map(({ label, value }) => (
                      <div key={label} className="text-center">
                        <p className="text-2xl font-extrabold" style={{ color: ORG }}>{value}</p>
                  ...
                  <div className="flex items-center gap-2">
                    <code className="flex-1 bg-gray-100 rounded-lg px-3 py-2 text-sm font-mono font-bold text-center tracking-widest" style={{ color: NAVY }}>
                      {referral.referralCode}
                    </code>
                    <button
                      onClick={() => navigator.clipboard.writeText(`${window.location.origin}/register?ref=${referral.referralCode}`)}
                      className="text-white text-xs font-bold px-3 py-2 rounded-lg transition hover:opacity-90"
                      style={{ background: ORG }}>
                      Copy
                    </button>
                  </div>
                  <Link href="/referral" className="block text-center text-xs font-semibold mt-3 hover:underline" style={{ color: ORG }}>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                ...
                {[
                  ...
                  { icon: '🎭', label: 'Role',          val: badge.label },
                  { icon: '📋', label: 'Total Listings',val: String(listings.length) },
                  { icon: '✅', label: 'Active',        val: String(myActive) },
                  { icon: '🪙', label: 'Coins',         val: user.coins.toLocaleString() },
                ].map(({ icon, label, val }) => (
                  <div key={label} className="flex items-center justify-between py-1.5 border-b border-gray-50 last:border-b-0">
                    <span className="text-xs text-gray-500 flex items-center gap-1.5"><span>{icon}</span>{label}</span>
                    <span className="text-xs font-semibold text-gray-800 truncate max-w-[140px]">{val}</span>
                  </div>
                ))}
                <Link href="/coins"
                  className="mt-4 w-full text-white text-xs font-bold py-2.5 rounded-lg transition hover:opacity-90 flex items-center justify-center"
                  style={{ background: NAVY }}>
                  🪙 Top Up Coins
                </Link>
              </div>
            ...
            {/* Staff/Admin: recent users & listings */}
            {isPriv && (
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-xs font-bold uppercase tracking-widest" style={{ color: NAVY }}>Recent Users</h3>
                    <Link href="/staff/users" className="text-xs font-semibold hover:underline" style={{ color: ORG }}>View all →</Link>
                  </div>
                  {recentUsers.length === 0 && <p className="text-xs text-gray-400">No users yet.</p>}
                  {recentUsers.map(u => (
                    <div key={u.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-b-0">
                      ...
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: '#fff7ed', color: ORG }}>
                        🪙 {u.coins}
                      </span>
                    </div>
                  ))}
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-xs font-bold uppercase tracking-widest" style={{ color: NAVY }}>Recent Listings</h3>
                    <Link href="/staff/listings" className="text-xs font-semibold hover:underline" style={{ color: ORG }}>View all →</Link>
                  </div>
                  {recentListings.length === 0 && <p className="text-xs text-gray-400">No listings yet.</p>}
                  {recentListings.map(l => (
                    <div key={l.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-b-0">
                      <div>
                        <p className="text-xs font-semibold text-gray-800 truncate max-w-[160px]">{l.title}</p>
                        <p className="text-[10px] text-gray-400">by {l.seller_name}</p>
                      </div>
        ...
        {/* ── AMBASSADOR CERTIFICATE (awarded on registration) ── */}
        <section id="ambassador-cert">
          <style>{`@import url('https://fonts.googleapis.com/css2?family=Dancing+Script:wght@700&display=swap');`}</style>
          <div className="flex items-center gap-3 mb-3">
            <div>
              <h2 className="text-xs font-bold uppercase tracking-widest" style={{ color: NAVY }}>
                🏅 Your Ambassador Certificate
              </h2>
              <p className="text-[10px] text-gray-400 mt-0.5">
                Awarded upon registration · View only — a staff member will print and hand this to you
              </p>
            </div>
            <div className="ml-auto flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium border" style={{ borderColor: '#e5e7eb', color: '#6b7280', background: '#f9fafb' }}>
              🔒 Print access: Staff only
            </div>
          </div>

          {/* Scrollable wrapper so it doesn't overflow on smaller screens */}
