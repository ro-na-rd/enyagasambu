    ...
    <div className="p-4 lg:p-8 animate-fadeInUp">
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-100">Users <span className="text-gray-500 text-base font-normal">({total})</span></h1>
            <p className="text-sm text-gray-500 mt-0.5">Manage all platform users &mdash; assign roles, grant coins</p>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/[0.04]" style={{ background: '#1c2333' }}>
                <th className="text-left px-4 py-3 text-gray-400 text-xs uppercase font-semibold tracking-wider">User</th>
                <th className="text-left px-4 py-3 text-gray-400 text-xs uppercase font-semibold tracking-wider">Phone</th>
                <th className="text-center px-4 py-3 text-gray-400 text-xs uppercase font-semibold tracking-wider">Coins</th>
                <th className="text-center px-4 py-3 text-gray-400 text-xs uppercase font-semibold tracking-wider">Role</th>
            ...
            <tbody className="divide-y divide-white/[0.04]">
              {fetching ? (
                ...
                <tr key={u.id} className="hover:bg-white/[0.02] transition" style={{ animationDelay: `${idx * 0.03}s` }}>
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center text-white text-xs font-bold shrink-0"
                        style={{ background: `linear-gradient(135deg, ${NAVY}, #162d6e)` }}>
                  ...
                  <td className="px-4 py-3.5 text-center">
                    <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full ${roleColors[u.role] || 'bg-gray-500/10 text-gray-500'}`}>
                      {u.role}
                    </span>
                  </td>
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-2 justify-center flex-wrap">
                      ...
                      {grantTarget === u.id ? (
                        <div className="flex items-center gap-1">
                          ...
                          <button onClick={() => setGrantTarget(null)} className="text-gray-500 text-xs hover:underline">X</button>
                        </div>
                      ) : (
