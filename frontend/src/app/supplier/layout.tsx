...
export default function SupplierLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!loading && user && user.role !== 'supplier' && !isAuthPage) router.replace('/login');
  ...
  const initials = user.name?.split(' ').map((p: string) => p[0]).join('').toUpperCase().slice(0, 2) || 'S';
    ...
    <div className="h-screen overflow-hidden flex" style={{ background: '#f0f2f6' }}>
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 w-64 bg-white border-r border-gray-200 flex flex-col transition-transform duration-200 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="px-3 py-3 border-t border-gray-100">
          <button onClick={() => { logout(); router.push('/supplier/login'); }}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-2 text-sm text-gray-400">
              <span className="text-gray-900 font-medium capitalize">
                {pathname === '/supplier' ? 'Dashboard' : pathname.split('/').pop()?.replace(/-/g, ' ') || ''}
              </span>
            </div>
          </div>
