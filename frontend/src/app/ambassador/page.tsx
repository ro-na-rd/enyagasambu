...
const ambassadorRoles = [
  { href: '/ambassador/profile',    icon: '👤',  label: 'My Profile',        desc: 'Manage your account information and contact details', color: '#0f1e42' },
  { href: '/ambassador/referrals',  icon: '🔗',  label: 'My Referrals',      desc: 'Refer users to the platform and earn coins', color: '#059669' },
  { href: '/ambassador/activities', icon: '📋',  label: 'My Activities',     desc: 'Track your actions, events, and achievements', color: '#d97706' },
  { href: '/ambassador/rewards',    icon: '🎁',  label: 'Rewards & Earnings', desc: 'View your coins, bonuses, and reward history', color: '#E85D04' },
  { href: '/ambassador/certificate',icon: '🏅',  label: 'My Certificate',    desc: 'Get your official ambassador certificate', color: '#7c3aed' },
  { href: '/ambassador/announcements',icon: '📢',label: 'Announcements',     desc: 'Stay updated with latest news and program updates', color: '#0891b2' },
  { href: '/ambassador/reports',    icon: '📈',  label: 'Reports',           desc: 'View your performance reports and analytics', color: '#be123c' },
  { href: '/ambassador/settings',   icon: '⚙️',  label: 'Settings',          desc: 'Configure your notification preferences', color: '#4b5563' },
  { href: '/ambassador/help',       icon: '❓',  label: 'Help & Support',    desc: 'FAQs, guides, and contact support team', color: '#1d4ed8' },
];

const certStatusConfig: Record<string, { label: string; icon: string; color: string; bg: string; action: string; href: string }> = {
  none:      { label: 'Not Requested', icon: '🏅', color: '#6b7280', bg: '#f3f4f6', action: 'Pay 2,000 RWF', href: '/ambassador/certificate' },
  pending:   { label: 'Payment Pending', icon: '⏳', color: '#d97706', bg: '#fffbeb', action: 'Complete Payment', href: '/ambassador/certificate' },
  paid:      { label: 'Payment Confirmed', icon: '✅', color: '#059669', bg: '#ecfdf5', action: 'View Certificate', href: '/ambassador/certificate' },
  generated: { label: 'Certificate Ready', icon: '🎉', color: '#0f1e42', bg: '#eef2ff', action: 'View Certificate', href: '/ambassador/certificate' },
};
...
export default function AmbassadorDashboardPage() {
  ...
  const stats = [
    { label: 'Total Referrals', value: referral?.totalReferrals ?? 0, icon: '👥', color: NAVY, bg: '#eef2ff' },
    { label: 'Successful', value: referral?.bonusPaid ?? 0, icon: '✅', color: '#059669', bg: '#ecfdf5' },
    { label: 'Pending', value: (referral?.totalReferrals ?? 0) - (referral?.bonusPaid ?? 0), icon: '⏳', color: '#d97706', bg: '#fffbeb' },
    { label: 'Rewards Earned', value: ((referral?.bonusPaid ?? 0) * (referral?.bonusPerReferral ?? 200)).toLocaleString(), icon: '🪙', color: ORG, bg: '#fff7ed' },
  ];

  const activities = [
    { action: 'You joined the Ambassador Program', time: 'Just now', icon: '🎉' },
    ...(referral && referral.totalReferrals > 0
      ? [{ action: `${referral.totalReferrals} referral(s) made`, time: 'Today', icon: '🔗' }]
      : []),
  ];
              ...
              {loading ? (
                ...
                <>
                  ...
                  <button
                    ...
                    className="w-full bg-[#E85D04] text-white text-sm font-bold py-2.5 rounded-lg hover:bg-[#c04a00] transition">
                    📋 Copy Referral Link
                  </button>
                </>
              )}
          ...
          <div>
            ...
            <Link href="/ambassador/certificate" className="block bg-white rounded-xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition group">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl shrink-0" style={{ background: certCfg.bg }}>{certCfg.icon}</div>
                <div className="min-w-0">
