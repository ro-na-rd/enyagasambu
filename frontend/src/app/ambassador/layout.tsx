...
const menuItems = [
  { href: '/ambassador',            icon: '📊', label: 'Dashboard' },
  { href: '/ambassador/profile',    icon: '👤', label: 'My Profile' },
  { href: '/ambassador/referrals',  icon: '🔗', label: 'My Referrals' },
  { href: '/ambassador/activities', icon: '📋', label: 'My Activities' },
  { href: '/ambassador/rewards',    icon: '🎁', label: 'Rewards & Earnings' },
  { href: '/ambassador/certificate', icon: '🏅', label: 'My Certificate' },
  { href: '/ambassador/announcements', icon: '📢', label: 'Announcements' },
  { href: '/ambassador/reports',    icon: '📈', label: 'Reports' },
  { href: '/ambassador/settings',   icon: '⚙️', label: 'Settings' },
  { href: '/ambassador/help',       icon: '❓', label: 'Help & Support' },
];
...
export default function AmbassadorLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!loading && !user && !isLoginPage) router.replace('/ambassador/login');
    if (!loading && user && user.role !== 'ambassador' && !isLoginPage) router.replace('/ambassador/login');
