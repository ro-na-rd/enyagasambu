  ...
  return (
    <div className="p-4 lg:p-8">
      ...
      <div className="grid lg:grid-cols-3 gap-4 mb-8">
        {[
          { label: 'Total Coins Earned', value: totalCoins, icon: <Coins size={24} />, sub: 'Including welcome bonus', color: ORG },
          { label: 'Referral Bonus', value: (referral?.bonusPaid ?? 0) * (referral?.bonusPerReferral ?? 200), icon: <Coins size={24} />, sub: `${referral?.bonusPaid ?? 0} successful referrals`, color: '#059669' },
          { label: 'Welcome Bonus', value: 100, icon: <Sparkles size={24} />, sub: 'Account registration', color: NAVY },
        ].map((s) => (
          <div key={s.label} className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            <div className="flex items-center justify-between mb-2">
              <span style={{ color: s.color }}>{s.icon}</span>
            </div>
            <p className="text-3xl font-extrabold" style={{ color: s.color }}>{s.value.toLocaleString()} <Coins size={18} /></p>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between py-3 border-b border-gray-50">
            <div className="flex items-center gap-3">
              <span style={{ color: NAVY }}><Sparkles size={20} /></span>
              <div>
            ...
            <span className="text-sm font-bold text-green-600">+100 <Coins size={14} /></span>
          </div>
          {referral && referral.bonusPaid > 0 && (
            <div className="flex items-center justify-between py-3 border-b border-gray-50">
              ...
              <span className="text-sm font-bold text-green-600">+{referral.bonusPaid * (referral.bonusPerReferral ?? 200)} <Coins size={14} /></span>
            </div>
          )}
        </div>
      </div>
