...
interface AmbassadorCertificate {
...
interface VerifyResult {
  valid: boolean;
  message?: string;
  certificate: AmbassadorCertificate;
}
...
export default function VerifyCertificatePage() {
  ...
  useEffect(() => {
    const verifyCertificate = async () => {
      try {
        const { data } = await api.get(`/ambassador/certificate/verify/${certNo}`);
        setResult(data);
      } catch (err: unknown) {
        ...
        {error ? (
          ...
          <div className="bg-white rounded-xl shadow-sm p-8 border border-green-200">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h2 className="text-xl font-bold text-green-900">Valid Certificate</h2>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-50 rounded-lg p-4">
                  <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Ambassador Name</p>
                  <p className="font-semibold text-gray-900">{result.certificate.ambassador_name}</p>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
              ...
              {result.certificate.ambassador_phone && (
                <div className="bg-gray-50 rounded-lg p-4">
                  <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Contact Phone</p>
                  <p className="font-semibold text-gray-900">{result.certificate.ambassador_phone}</p>
                </div>
              )}
              ...
              {result.certificate.is_expired && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                  <p className="text-sm text-yellow-800">
                    <strong>Note:</strong> This certificate has expired. Please contact the ambassador or our support team for renewal information.
                  </p>
                </div>
              )}
            </div>
