...
exports.getCertificates = async (req, res) => {
  ...
  try {
    ...
    const [[{ total }]] = await pool.query(
      `SELECT COUNT(*) AS total FROM supplier_certificates sc JOIN users u ON sc.user_id = u.id WHERE ${where}`,
      countParams
    );

    const [rows] = await pool.query(
      `SELECT sc.*, u.name AS user_name, u.email AS user_email, u.phone AS user_phone,
...
exports.generateCertificate = async (req, res) => {
  ...
  try {
    const [[cert]] = await pool.query(
      'SELECT sc.*, u.name AS user_name FROM supplier_certificates sc JOIN users u ON sc.user_id = u.id WHERE sc.id = ?',
      [id]
    );
...
exports.confirmPayment = async (req, res) => {
  ...
  try {
    const [[cert]] = await pool.query(
      'SELECT id, status FROM supplier_certificates WHERE id = ?',
      [id]
    );
    if (!cert) return res.status(404).json({ message: 'Certificate request not found' });
    if (cert.status === 'generated') return res.status(400).json({ message: 'Certificate already generated' });
    ...
    await pool.query(
      'UPDATE supplier_certificates SET status = ? WHERE id = ?',
      ['paid', id]
    );
  ...
  try {
    const [[cert]] = await pool.query(
      `SELECT sc.*, u.name AS user_name, u.email AS user_email, u.phone AS user_phone,
       ...
       FROM supplier_certificates sc
       JOIN users u ON sc.user_id = u.id
       LEFT JOIN staff s ON sc.generated_by = s.id
       LEFT JOIN certificate_types ct ON sc.certificate_type_id = ct.id
       WHERE sc.id = ?`,
      [id]
    );
...
exports.bulkUpdateCertificates = async (req, res) => {
  ...
  try {
    ...
    logger.error('[Admin bulkUpdateSupplierCertificates error]', err);
    return res.status(500).json({ message: 'Server error' });
  }
};
