...
exports.verifyOtp = async (req, res) => {
  ...
  try {
    ...
    const [[staffInfo]] = await pool.query('SELECT username, phone, role FROM staff WHERE id = ?', [otpRow.staff_id]);

    const token = jwt.sign(
      { id: otpRow.staff_id, username: staffInfo?.username, phone: otpRow.phone, role: otpRow.role, is_staff: true },
      process.env.JWT_SECRET,
      { expiresIn: '8h' }
    );

    return res.json({ token, role: otpRow.role, user: { id: otpRow.staff_id, name: staffInfo?.username || 'Admin', email: staffInfo?.username, phone: otpRow.phone, role: otpRow.role } });
  } catch (err) {
...
exports.me = async (req, res) => {
  try {
    const [[staff]] = await pool.query(
      'SELECT id, username AS name, username AS email, phone, role FROM staff WHERE id = ? AND is_active = 1',
      [req.user.id]
    );
