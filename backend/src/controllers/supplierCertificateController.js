...
async function resolveType(certificateTypeId) {
  ...
  if (type) return type;
  return { id: null, code: 'SUPPLIER', name: 'Verified Supplier', price_rwf: DEFAULT_CERT_PRICE, duration_years: 1 };
}

exports.getMyCertificate = async (req, res) => {
  try {
    const [[userRow]] = await pool.query(
      `SELECT u.id, u.name, u.email, u.phone, u.coins, u.created_at AS registered_date,
              sp.business_name, sp.business_phone
       FROM users u
    ...
    let [rows] = await pool.query(
      `SELECT sc.id, sc.photo_url, sc.cert_no, sc.status, sc.payment_ref, sc.amount_rwf,
              sc.issued_date, sc.valid_until, sc.created_at, sc.updated_at, sc.certificate_type_id,
       ...
       WHERE sc.user_id = ? ORDER BY sc.created_at DESC LIMIT 1`,
      [req.user.id]
    );
    ...
    if (rows.length === 0) {
      ...
      const [result] = await pool.query(
        'INSERT INTO supplier_certificates (user_id, status, amount_rwf, certificate_type_id) VALUES (?, ?, ?, ?)',
    ...
    return res.json({
      certificate: {
        ...cert,
        supplier_name: userRow?.name || '',
        supplier_email: userRow?.email || '',
        supplier_phone: userRow?.phone || '',
        business_name: userRow?.business_name || null,
        supplier_photo: cert.photo_url || null,
      },
    });
  } catch (err) {
    logger.error(err);
...
exports.uploadPhoto = async (req, res) => {
  try {
    ...
    const [existing] = await pool.query(
      'SELECT id, status FROM supplier_certificates WHERE user_id = ? ORDER BY created_at DESC LIMIT 1',
      [req.user.id]
    );

    if (existing.length > 0 && existing[0].status === 'pending') {
      await pool.query('UPDATE supplier_certificates SET photo_url = ? WHERE id = ?', [photoUrl, existing[0].id]);
      return res.json({ message: 'Photo updated', certificateId: existing[0].id, photo_url: photoUrl });
    }

    const [result] = await pool.query(
      'INSERT INTO supplier_certificates (user_id, photo_url, status) VALUES (?, ?, ?)',
...
exports.initiatePayment = async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'UPDATE supplier_certificates SET payment_ref = ?, amount_rwf = ?, certificate_type_id = ? WHERE id = ?',
    ...
    await requestToPay({
      ...
      payerMessage: 'Supplier Certificate Fee',
    ...
    logger.error('[Supplier Cert MoMo initiate error]', err?.response?.data || err.message);
...
exports.checkPayment = async (req, res) => {
  ...
  try {
    const [[cert]] = await pool.query(
      'SELECT id, status, payment_ref FROM supplier_certificates WHERE payment_ref = ? AND user_id = ?',
    ...
    if (momoStatus.status === 'SUCCESSFUL') {
      ...
      await pool.query(
        'UPDATE supplier_certificates SET status = ?, cert_no = ?, issued_date = ?, valid_until = ? WHERE id = ?',
    ...
    if (momoStatus.status === 'FAILED') {
      await pool.query(
        'UPDATE supplier_certificates SET payment_ref = NULL WHERE id = ?',
        [cert.id]
      );
    ...
    logger.error('[Supplier Cert MoMo check error]', err?.response?.data || err.message);
...
exports.verifyCertificate = async (req, res) => {
  ...
  try {
    ...
    logger.error('[Supplier certificate verification error]', err);
