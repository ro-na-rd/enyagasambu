...
exports.register = async(req, res) => {
    ...
    try {
        ...
        const [result] = await conn.query(
            'INSERT INTO users (name, email, phone, password_hash, coins, role) VALUES (?, ?, ?, ?, ?, ?)', [name, email, phone || null, password_hash, 0, 'broker']
        );

        await conn.query(
            'INSERT INTO broker_certificates (broker_id, status, amount_rwf) VALUES (?, ?, ?)', [result.insertId, 'pending', CERT_PRICE]
        );
        ...
        const [
            [user]
        ] = await pool.query('SELECT coins FROM users WHERE id = ?', [result.insertId]);
...
exports.login = async(req, res) => {
    ...
    try {
        const [rows] = await pool.query(
            'SELECT id, name, email, phone, password_hash, coins, role FROM users WHERE email = ? AND role = ?', [email, 'broker']
        );
